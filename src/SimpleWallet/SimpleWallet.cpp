...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 13007", 0 };
  ...
  bool parseArguments(LoggerRef& logger, const std::vector<std::string> &args) {
    ...
    try {
      ...
      if (fake_outs_count > m_currency.maxMixin()) {
          logger(ERROR, BRIGHT_RED) << "mixin should be equal or less than" << m_currency.maxMixin();
          return false;
      }
